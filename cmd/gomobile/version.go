...
func runVersion(cmd *command) (err error) {
	// Check this binary matches the version in github.com/vina-bearvpn/mobile/cmd/gomobile
	...
	version, err := func() (string, error) {
		...
		cmd := exec.Command("go", "list", "-f", "{{.Stale}}", "github.com/vina-bearvpn/mobile/cmd/gomobile")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command("go", "list", "-f", "{{.Dir}}", "github.com/vina-bearvpn/mobile/app").CombinedOutput()
	if err != nil {
